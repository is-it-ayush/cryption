...
async function main() {
    ...
    const encrypted_buffer = await cryption.encryptAES(my_key, Buffer.from(my_string), 'AES-GCM', my_iv);
    const decrypted_string = await cryption.decryptAES(my_key, encrypted_buffer, 'AES-GCM', my_iv);
