 ...
 */
export async function sign_with(key: crypto.webcrypto.CryptoKey, data: Buffer, algorithm: string, hash: HashType = "SHA-256", saltLength: number = 32) {
    ...
    switch (algorithm) {
        ...
        case 'RSA-PSS'.toLowerCase():
            alg = { name: "RSA-PSS", saltLength: saltLength } as crypto.webcrypto.RsaPssParams;
            break;
        default:
            alg = { name: "HMAC" }
            break;
 ...
 * @returns `ArrayBuffer`- The signature in ArrayBuffer
 */
export async function verify_with(key: crypto.webcrypto.CryptoKey, data: Buffer, algorithm: string, signature: Buffer, hash: HashType = "SHA-256", saltLength: number = 32) {
